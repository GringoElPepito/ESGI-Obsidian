...
Mes missions se concentrent principalement sur les automatisations Ansible, cependant énormément de personnes différentes, prestataires externes ou internes à l'équipe, ont créer et/ou modifier les automatisations existantes à ce-jour. A cause de cela, le code ne respecte pas de standard ce qui complexifie le maintien de la base de code existante. C'est pourquoi depuis mon arrivée, je tente autant que possible d'uniformiser le code de nos différentes automatisations. Que ce soit avec la création de rôle standard pour Ansible permettant d'éviter la duplication du code
