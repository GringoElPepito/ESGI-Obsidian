...
L'Hôtel COSTES a été inauguré en 1995, à cette époque, celui-ci ne contient que la partie Saint-Honoré nom défini à partir de la rue adjacente à cette partie de l'hôtel. Cette section de l'hôtel est le fruit de la collaboration entre Jean-Louis COSTES et le designer Jacques GARCIA, elle se définit par son apparence singulière avec des teintes plutôt orientés sur le pourpre et le rouge bordeaux ainsi qu'un style tout particulier reprenant certains codes. La partie Saint-Honoré regroupe les chambres ainsi que les petites suites.
En 2004, l'Hôtel COSTES s'étendit avec l'acquisition de l'Hôtel le Lotti situé 7 rue Castiglione, qui est par ailleurs aujourd'hui l'entrée principale de l'hôtel. Castiglione est bien évidemment le nom qui a été donné à cette partie de l'hôtel. Cette fois-ci Jean-Louis COSTES a fait appel Christian LIAIGRE pour se charger du design. 
...
- Je supervise le fonctionnement de nos logiciels de production ainsi que leur interfaçage pour garantir le bon déroulement des étapes de la production.
...
Nous créons chaque jour des ToDo List nous permettant ainsi d'avoir un fil à suivre au cours de la journée, ce qui est très pratique surtout lorsque nous avons du gérer plusieurs pannes à la suite ce qui peut nous sortir de notre tâche. Les ToDo sont là pour nous permettre de facilement reprendre là où nous nous sommes arrêtés.
...
Pour pouvoir soutenir au mieux la production, les hôtels mettent en place des outils PMS (Property Management System) qui est une solution logiciel permettant à un hôtel ou à un groupe hôtelier de gérer facilement les différentes étapes de la production comme la réception, la réservation, la tarification des chambres ou encore la facturation. Dans notre cas, la solution PMS actuellement en place au sein de l'Hôtel COSTES est Opera Cloud.
Pour les boutiques et restaurant on privilégie les solutions de type POS (Point of Sales), qui est un système centralisant toutes les transactions liés à l'activité. Etant donné que l'Hôtel possède aussi un restaurant, une boutique et une boutique éphémère, une solution de POS est en place pour soutenir ces 3 activités. La solution utilisé est Simphony, qui encore une fois proposé par Oracle et directement interfacé avec Opéra. De cette manière, toutes les transactions enregistrés dans Simphony sont retranscrites dans Opéra permettant ainsi d'enregistrer le résultat générer par toutes les activités de l'établissement.
Ce n'est pas tout, Opéra est interfacé avec 

Dans un hôtel, le contrôle d'accès est un sujet qu'il ne faut surtout pas négliger. Dans notre cas, nous utilisons la solution Guest key qui est serveur centralisant les évènements ainsi que les permissions des cartes/clé optiques enregistrer dans le système. Ce serveur communique avec les badgeuses et/ou lecteur optiques pour vérifier que la personnes souhaitant accéder à un endroit spécifique est en possession des permissions requises. Ce serveur propose une interface graphique sous la forme d'un client lourd installé directement sur le serveur, permettant de gérer l'intégralité du parc de contrôle d'accès. En plus, du serveur en local, il existe la solution Guest Key Cloud qui est une interface web accessible depuis n'importe quel navigateur qui communique avec le serveur Guest Key local, ce qui évite de devoir donné une connexion direct au serveur local pour les utilisateurs amenés à créer des cartes d'accès.
Nous sommes en contact direct avec l'entreprise anglaise Guest Key à l'origine de la solution, qui nous vient en aide lorsque nous rencontrons une panne.
...
Concernant la partie virtualisation, pour cela nous avons 3 serveurs ESXi tous indépendant, qui virtualise nos différents serveurs à savoir, Omnivista 2500 & 8770, les serveurs OXE, le serveur Guest Key ainsi que le serveur d'interfaçage.
...
- Spa Booker par pure informatique
- Book4Time par agilsys
- ???
...
Après évaluation des 3 solutions nous avons optés pour Spa Booker. les 3 solutions étaient plutôt équivalente en termes de fonctionnalités, ce qui a fait la différence est le prix et aussi la durée de mise en place qui est de 4 semaines contre 6 à 8 semaines pour les 2 autres solutions.
...
L'entreprise SCA SAINT HONORE et plus particulièrement l'Hôtel COSTES se base sur de fortes personnalités, la principale étant bien évidemment Jean-Louis COSTES fondateur de l'hôtel éponyme. En effet, l'hôtel ne possède à ce jour pas de service marketing ou commercial, la clientèle de l'hôtel s'est principalement construite autour de la réputation de M. COSTES. Peu d'établissement peuvent se vanter d'une telle prouesse et M. COSTES n'est pas la seule personnalité sur laquelle se repose l'Hôtel COSTES, il y a aussi Mme. Taly NAINVILLE Directrice de l'Hébergement, MM. Clément & François LAUMONIER qui sont respectivement Directeur du restaurant & Directeur du Room Service ou encore M. Radu Daniel ILIE Directeur des Systèmes d'Informations qui tous contribuent à l'amélioration et la renommé de l'Hôtel. L'organisation au sein de l'hôtel est assez classique, avec un système oligarchique que l'on pourrait retrouver dans la plupart des entreprises.

Par contre en ce qui concerne le service informatique, celui-ci est organisé holacratie, entendez que toutes les personnes du service possèdent un poids égale sur la prise de décision. Ceci est bien évidemment réalisable car le service informatique ne compte que 3 personnes, de plus, cela n'empêche pas l'existence d'une relation de subordination avec M. ILIE. Malgré tout, il est quand même valorisant de voir que même si je suis alternant mon avis est entendu et pris en compte et ce même si celui-ci n'est pas parfait et qu'il présente des défauts. Cela nous force à échanger et à comprendre la vision de l'autre pour ensuite trouver un terrain d'entente et répondre à la problématique initiale.
Sur la répartition des tâches, nous nous répartissons les différentes tâches à notre arrivée et les inscrivons dans la ToDo List du jour, ainsi nous pouvons tous travailler de manière indépendante ce qui s'avère littéralement indispensable au vu de notre effectif.
Cette effectif s'est par ailleurs assez souvent avéré handicapant. En effet, pour qu'un service assure une activité 7j/7, il faut un minimum de 5 personnes, dans notre cas nous sommes seulement 3 ce qui peut nous amener a être assez rapidement débordé lorsque l'on retrouve seul sur site. 
Un des éléments principaux qui a permis de maintenir notre qualité de service en dépit de cette faiblesse numérique, c'est l'organisation mise en place par M. ILIE. C'est surement le point le plus important ayant maintenu à flot le service informatique, l'hôtellerie de luxe est un domaine très exigeant demandant une très grande réactivité et pouvant très vite nous dépassé si on y est mal préparé. C'est pour cette raison que M. ILIE est très soucieux de l'organisation globale du service et de transmettre ce sens de l'organisation à ses collègues. 
...
À mon arrivée, cela a été quelque chose d'assez compliqué pour moi à appréhender, car dans mon expérience précédente nous réalisions tout par nous-même. J'ai donc dû m'adapter à cette nouvelle manière de travailler en comprenant qu'il était parfois plus pertinent de délégué une tâche dans le but de gagner en efficacité. 
Cependant voyant que mon profil était plus porté sur le technique, M. ILIE a décider d'internaliser certaines tâches comme le packaging des applications, la configuration des switches ou encore les tâches de Scripting. Ceci nous a permit de renégocier certains contrat de maintenance à la baisse, ce qui représente une économie non-négligeable. Mais ce n'est pas tout, j'ai aussi pu aider M. ILIE concernant des prises de décisions purement technique, notamment sur la méthode la plus adapté pour la gestion de nos imprimantes. Cette décision à d'ailleurs conduit à un projet que j'ai moi-même mené à bien et réalisé, celui-ci consistait en l'installation d'un serveur d'impression ce qui implique l'adaptation de l'infrastructure existante avec par exemple l'ajout d'un vlan et d'un DNS internaliser. Bien évidemment, M.ILIE aurait pu simplement demander à l'un de nos prestataires de s'en charger, la différence aurait été que nous n'aurions eu aucune maitrise de cette partie de notre infrastructure ce qui aurait pu nous rendre dépendant de notre prestataire en plus de nous ajouter des coût supplémentaire.
Ma présence a permis de rehausser le capital technique du service informatique, même s'il est vrai que faire appel aux experts de nos prestataires seraient bien plus efficace dans la grande majorité des cas, il est tout de même important de connaître son infrastructure et de maitriser cette dernière.
...
Même si cette alternance n'est pas vraiment porté sur la technique, j'ai tout de même pu apprendre énormément de concept et acquérir de nouvelles compétences, notamment en échangeant avec nos différents prestataires. Les intervenants nous étant envoyés, sont généralement des experts spécialisés dans un domaine spécifique, de plus, ces deniers sont très pédagogues et n'hésite pas expliquer leurs actions lorsqu'ils sont en intervention. Ceci m'a été très précieux, je suis quelqu'un de passionné par la technique et c'était un de points me faisant un peu peur, j'appréhendais le fait que les missions finisse par me lasser. Heureusement ça n'a pas été le cas, les missions variés et les échangent avec nos prestataires ne m'en ont pas laisser le temps. Même si je n'ai pas forcément pu pratiquer d'un point de vue purement technique, j'ai pu observer attentivement ces experts intervenir, ce qui m'a très certainement plus appris que si j'avais du réalisé leurs tâches par moi-même. J'ai pu par exemple découvrir les solutions Alcatel pour le réseau et la téléphonie, approfondir mes connaissances de Microsoft 365 et Intune ou encore compétences en terme de gestion de projet.
J'ai aussi pu développer mon rapport aux autres, le domaine de l'hôtellerie pousse davantage aux interactions spontanées, ce qui est pour moi de base quelque chose d'assez compliqué, étant plutôt réservé de nature. C'est pourquoi cette alternance m'a été d'une grande aide sur ce point, j'ai du interagir avec des prestataires et des clients venants des 4 coins du monde ce qui m'a donc en plus permis de travailler mon anglais. J'ai désormais bien plus d'aisance à en lancer une conversation ou aborder une personne ce qui est une amélioration non-négligeable. 
...
J'ai poursuivi, en m'attardant cette fois-ci sur le service informatique, en expliquant le rôle du service les missions qui en découlaient ainsi que notre organisation au quotidien. Cette section vise à vous faire comprendre les attentes reposant sur ce service ainsi que sa dynamique générale.

La seconde partie du rapport se consacre à l'explication de mes tâches quotidiennes au cours de cette alternance en tant que technicien IT. J'ai pu vous détailler les différentes missions que j'ai réalisées ainsi que les projets sur lesquels j'ai eu l'occasion de travaillé. Ce passage vous permet de illustrer les différentes tâches que j'ai du accomplir, les problématiques qui m'ont fait face ainsi que la manière utiliser pour y pouvoir y répondre.

Pour résumé, ce rapport est le déroulement de mon expérience qu'a été mon alternance au sein de l'entreprise SCA SAINT HONORE, mettant à l'honneur le résultat de mes travaux, les compétences acquises, les projets réalisés ou encore les collaborations offert par le contexte de cette entreprise, poussant à l'adaptation et à l'évolution de manière constante.  